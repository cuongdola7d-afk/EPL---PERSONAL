...
Hai fixture Gameweek 1 `1208021` (Manchester United–Fulham) và `1208022` (Ipswich–Liverpool) có file bằng chứng API-Football rút gọn trong `data/fixture-<id>-evidence.json`. Lệnh nhập một lần lưu ID bàn thắng/kiến tạo, người vào/ra sân và đội hình vào `fixture_score_evidence` theo khóa fixture ID. Trước khi suy luận, backend kiểm tra 40 ID, 11 đá chính + 9 dự bị mỗi đội, người vào sân có phút trong H2, bàn thắng/kiến tạo khớp dữ liệu dương và sự kiện bàn thắng khớp tỉ số. Nếu kiểm tra sai, API trả `evidenceStatus=INVALID`, `evidenceError` và điểm vẫn tạm tính. Khi đúng, `inferred.minutes/goals/assists` chứa riêng giá trị suy luận; các trường gốc trong `fixture_player_stats` và JSON chi tiết vẫn giữ `null`. Chạy lại cùng file cập nhật một dòng bằng chứng, không tạo dòng trùng. Fixture Gameweek 1 chưa có bằng chứng vẫn giữ điểm tạm tính. Lệnh local từ `backend/` (chọn file tương ứng):
